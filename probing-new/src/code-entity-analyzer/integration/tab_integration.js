...
class TabIntegration {
  ...
  renderComponentGraph(tabInfo, container) {
    try {
      // Import the component graph module
      const { createComponentGraph } = require('../visualizations/component_graph');
      
      // Create the visualization
      const visualization = createComponentGraph(
        tabInfo.data,
        container.id,
        tabInfo.options
      );
      ...
      tabInfo.visualization = visualization;
  ...
  renderStateFlow(tabInfo, container) {
    try {
      // Import the state flow diagram module
      const { createStateFlowDiagram } = require('../visualizations/state_flow_diagram');
      
      // Create the visualization
      const visualization = createStateFlowDiagram(
        tabInfo.data,
        container.id,
        tabInfo.options
      );
  ...
  renderDependencyHeatmap(tabInfo, container) {
    try {
      // Import the dependency heatmap module
      const { createDependencyHeatmap } = require('../visualizations/dependency_heatmap');
      
      // Create the visualization
      const visualization = createDependencyHeatmap(
        tabInfo.data,
        container.id,
        tabInfo.options
      );
