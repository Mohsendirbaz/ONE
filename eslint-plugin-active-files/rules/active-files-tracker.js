...
module.exports = {
  ...
  create(context) {
    ...
    // Use the PROJECT_BASE_DIR environment variable if set, otherwise fall back to process.cwd()
    const projectRoot = process.env.PROJECT_BASE_DIR || process.cwd();
    ...
    return {
      Program(node) {
        ...
        // Use path.normalize to handle different path formats and case-insensitivity on Windows
        if (path.normalize(currentFilePath).toLowerCase() === path.normalize(entryPointAbsolute).toLowerCase()) {
          ...
          const collectFiles = (dir, projectFiles, isRootCollection = false) => {
            try {
              // Skip processing if this is a node_modules directory or similar
              const dirName = path.basename(dir);

              // Expanded list of directories to skip
              const skipDirs = [
                "node_modules", ".git", ".vscode", "build", "dist", "coverage", 
                "public", "static", "assets", "images", "docs", "test", "tests",
                "e2e", "cypress", "jest", "mocks", "__mocks__", "__tests__"
              ];

              // Skip processing this directory if it's in the skip list
              if (skipDirs.includes(dirName) || dirName.startsWith(".")) {
                return;
              }

              // For root collection, also skip Active-Inactive-Marking directory
              if (isRootCollection && dirName === "Active-Inactive-Marking") {
                return;
              }
              ...
              for (const file of files) {
                ...
                if (file.isDirectory()) {
                  // Pass the isRootCollection flag to recursive calls
                  collectFiles(filePath, projectFiles, isRootCollection);
                  ...
                  // Skip files in node_modules (additional check for safety)
                  if (filePath.includes('node_modules')) {
                    continue;
                  }

                  projectFiles.add(filePath);
                  console.log(`Added file to analysis: ${path.relative(projectRoot, filePath)}`);
                }
              }
            } catch (error) {
          ...
          if (fs.existsSync(srcDir)) {
            collectFiles(srcDir, projectFiles);
          } else {
            console.warn(`Source directory not found: ${srcDir}`);
          }

          // Also collect files from other common directories
          const otherDirs = ["components", "utils", "lib", "helpers", "services", "api"];
          for (const dir of otherDirs) {
            const dirPath = path.join(projectRoot, dir);
            if (fs.existsSync(dirPath)) {
              collectFiles(dirPath, projectFiles);
            }
          }

          // If no files found yet, try collecting from the project root
          if (projectFiles.size === 0) {
            console.warn("No files found in standard directories, collecting from project root");
            collectFiles(projectRoot, projectFiles, true); // Pass true for isRootCollection
          }

          // Ensure the entry point file is included in the analysis
          if (fs.existsSync(entryPointAbsolute)) {
            projectFiles.add(entryPointAbsolute);
            console.log(`Ensuring entry point is included: ${path.relative(projectRoot, entryPointAbsolute)}`);
          } else {
            console.warn(`Entry point file not found: ${entryPointAbsolute}`);
          }

          // Log the total number of files found
          console.log(`Total files found for analysis: ${projectFiles.size}`);

          // If still no files found, this is likely an error
          if (projectFiles.size === 0) {
            console.error("ERROR: No files found for analysis. This is likely an error in the file collection process.");
            console.error("Adding the entry point file as a minimum to prevent empty report.");

            // Create the entry point file if it doesn't exist
            try {
              if (!fs.existsSync(path.dirname(entryPointAbsolute))) {
                fs.mkdirSync(path.dirname(entryPointAbsolute), { recursive: true });
              }

              if (!fs.existsSync(entryPointAbsolute)) {
                fs.writeFileSync(entryPointAbsolute, '// Auto-generated entry point\n', 'utf8');
                console.log(`Created entry point file: ${entryPointAbsolute}`);
              }

              projectFiles.add(entryPointAbsolute);
            } catch (error) {
              console.error(`Failed to create entry point file: ${error.message}`);
            }
          }
          ...
          // Count lines in each file and store the information
          console.log("Counting lines in each file...");
          const fileLinesCount = {};
          const fileSizes = {};

          // Function to count lines in a file
          const countLinesInFile = (file) => {
            try {
              // Skip if already processed
              const relativeFile = path.relative(projectRoot, file);
              if (fileLinesCount[relativeFile]) {
                return;
              }
              ...
              fileLinesCount[relativeFile] = lineCount;

              // Get file size in bytes
              const stats = fs.statSync(file);
              fileSizes[relativeFile] = stats.size;

              console.log(`  ${path.basename(file)}: ${lineCount} lines, ${stats.size} bytes`);
            } catch (error) {
              console.error(`Error counting lines in ${file}:`, error.message);

              // Set default values if file can't be read
              const relativeFile = path.relative(projectRoot, file);
              if (!fileLinesCount[relativeFile]) {
                fileLinesCount[relativeFile] = 0;
                fileSizes[relativeFile] = 0;
                console.warn(`  Using default values for ${path.basename(file)}: 0 lines, 0 bytes`);
              }
            }
          };

          // Process all project files to count lines and get file sizes
          for (const file of projectFiles) {
            countLinesInFile(file);
          }

          // Ensure entry point file has line count
          if (!fileLinesCount[path.relative(projectRoot, entryPointAbsolute)]) {
            console.log(`Counting lines in entry point file: ${path.relative(projectRoot, entryPointAbsolute)}`);
            countLinesInFile(entryPointAbsolute);
          }

          // Ensure all active files have line counts
          for (const file of activeFiles) {
            const relativeFile = path.relative(projectRoot, file);
            if (!fileLinesCount[relativeFile]) {
              console.log(`Counting lines in active file: ${relativeFile}`);
              countLinesInFile(file);
            }
          }
          ...
          const reportData = {
            ...
            fileLinesCount: fileLinesCount,
            fileSizes: fileSizes
          };

          // Validate the analysis results
          if (activeFiles.size === 0 || !activeFiles.has(entryPointAbsolute)) {
            console.error("ERROR: Entry point not marked as active. This is likely an error in the analysis.");
            ...
            console.log(`Manually added entry point to active files: ${path.relative(projectRoot, entryPointAbsolute)}`);
          ...
          try {
            ...
            const reportFilePath = path.join(absoluteProjectRoot, 'active-files-report.json');

            console.log(`Writing report to: ${reportFilePath}`);
            ...
            if (error.code === 'ENOENT') {
              console.error("Directory does not exist. Please ensure the project root directory is correct.");
            } else if (error.code === 'EACCES') {
              console.error("Permission denied. Please ensure you have write access to the project root directory.");
            }
          }
